    ...
    describe CsrfMetaTags, type: :tag do
      ...
      it "returns an empty string when Rails has no CSRF or yielded content" do
        ...
        template = Liquid::Template.parse(
          "{% csrf_meta_tags %}{% yield absent %}",
          environment: Liquid::Rails.environment
        )

        expect(template.render!(nil, registers: {view: view})).to eq("")
      end

      it "returns strings from generic tags when the view yields no content" do
        allow(view).to receive(:csrf_meta_tags).and_return(nil)
        allow(view).to receive(:content_for).and_return(nil)
        parse_context = Liquid::ParseContext.new(environment: Liquid::Rails.environment)

        expect(CsrfMetaTags.send(:new, "csrf_meta_tags", "", parse_context).render(context)).to eq("")
        expect(YieldTag.send(:new, "yield", "absent", parse_context).render(context)).to eq("")
      end
    end
  end
end
